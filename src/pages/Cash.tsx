...
const transactionDisplayText = (row: { operation_type?: string; title?: string; notes?: string }) => {
  if (row.operation_type !== "subscription_payment") return { title: row.title, notes: row.notes };
  const planName = (row.title || "Абонемент").replace(/^Доплата(?: за абонемент)?:\s*/i, "").trim() || "Абонемент";
  return { title: `Доплата: ${planName}`, notes: `Доплата за абонемент: ${planName}` };
};
